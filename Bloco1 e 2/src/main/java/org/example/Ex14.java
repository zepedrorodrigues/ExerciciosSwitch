...
public class Ex14 {
    public static int[] devolvernumerodumarray(int num, int numeroDireita){
        ...
        while(num!=0){
            ...
            new_Array[0] = alg;
            num=num/10;}
        new_Array = estenderArrayUmaCasaEsquerda(new_Array);
        new_Array[new_Array.length-1] = numeroDireita;
        return new_Array;}
    public static int[] estenderArrayUmaCasaDireita(int[] num_final){
        ...
        return new_Array;}
    public static int[] estenderArrayUmaCasaEsquerda(int[]num_final){
        int[] new_Array = new int[num_final.length+1];
        for(int i= 0; i< num_final.length;i++)
            new_Array[i]=num_final[i];
        return new_Array;}
