...
public class Ex22 {
    public static int[][] matrizBase(int[][] matrizSudoku) {
        if (!verificarSudokuTamanhoeNumeros(matrizSudoku) || !verificarSudokuTamanhoeNumeros(matrizSudoku))
            return new int[0][0];
    ...
    private static boolean testarColuna(int[][]matrizSudoku,int coluna, int numero){
        for(int i=0;i<matrizSudoku.length;i++){
            if(matrizSudoku[i][coluna-1]==numero)
                return false;}
        return true;}
    private static int escolherQuadrado(int[][]matrizSudoku,int linha,int coluna){
    ...
    private static boolean testarNumeroRepetiroQuadrado(int[][]matrizSudoku,int quadrado,int numero){
        ...
        for(int i=0;i<matrizSudoku.length;i++){
            for(int j=0;j<matrizSudoku[i].length;j++){
                if(escolherQuadrado(matrizSudoku,i+1,j+1)==quadrado){
                    quadradoEscolhido[contador]=matrizSudoku[i][j];
                    contador++;}}}
        for(int i=0;i<quadradoEscolhido.length;i++){
            if(quadradoEscolhido[i]==numero)
                return false;}
        return true;}
    ...
    public static int[][] adicionarNovoNumeroSudoku(int[][] matrizSudoku, int linha, int coluna, int numero) {
        ...
        if(testarColuna(matrizSudoku,coluna,numero)&&
                ...
                testarNumeroRepetiroQuadrado(matrizSudoku,escolherQuadrado(matrizSudoku,linha,coluna),numero)){
